...
void JurassicUniverse::addWorld(
	const string &name,
	shared_ptr<JurassicWorld> world)
{
...
shared_ptr<JurassicWorld> JurassicUniverse::getWorld(const string &name)
{
...
SynthesizerWithNames JurassicUniverse::toSynthesizer()
{
	if (_defaultWorld)
	{
		...
		for (auto &w : _worlds)
		{
			auto newW = _defaultWorld->copy();
			newW->addAllRexesFrom(w.second);
			w.second = newW;
		}
	}
	else
